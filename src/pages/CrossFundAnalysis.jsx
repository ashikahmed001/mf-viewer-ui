...
function StockDiscovery() {
  ...
  const [trend,     setTrend]     = useState('all'); // 'all'|'rising'|'peaked'|'exiting'|'exited'
  ...
  const stocks = useMemo(() => {
    ...
    return [...byIsin.values()]
      ...
      .map(s => {
        ...
        const prevFunds   = tl[tl.length - 2]?.fund_count ?? 0;
        return { ...s, tl, velocity, latestFunds, prevFunds };
      })
      .filter(s => {
        if (search   && !s.stock_name.toLowerCase().includes(search.toLowerCase())) return false;
        if (industry && s.industry !== industry) return false;
        if (trend === 'rising')  return s.latestFunds > s.prevFunds;
        if (trend === 'peaked')  return s.latestFunds === s.peak_funds && s.latestFunds > 0;
        if (trend === 'exiting') return s.latestFunds > 0 && s.latestFunds < s.peak_funds;
        if (trend === 'exited')  return s.latestFunds === 0;
        return true;
      })
      .sort((a, b) => {
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 w-44 focus:outline-none focus:ring-1 focus:ring-violet-300 focus:border-violet-400"
            />
          </div>
          <select
            value={industry}
            onChange={e => setIndustry(e.target.value)}
            className="text-xs border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 focus:outline-none focus:ring-1 focus:ring-violet-300 focus:border-violet-400"
          >
            <option value="">All industries</option>
            {industries.map(ind => <option key={ind} value={ind}>{ind}</option>)}
          </select>
          {(search || industry || trend !== 'all') && (
            <button
              onClick={() => { setSearch(''); setIndustry(''); setTrend('all'); }}
          ...
          <span className="ml-auto text-xs text-slate-400 dark:text-slate-500">{stocks.length} stock{stocks.length !== 1 ? 's' : ''}</span>
        </div>
        {/* Row 2: trend + min peak + sort */}
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
            ...
            {[
              ...
              { id: 'peaked',  label: '◆ At peak' },
              { id: 'exiting', label: '↓ Exiting' },
              { id: 'exited',  label: '✕ Exited' },
            ].map(t => (
              <button key={t.id} onClick={() => setTrend(t.id)}
                className={`px-2.5 py-1 rounded-lg transition-colors ${trend === t.id ? 'bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300 font-semibold' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
                {t.label}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
            <span className="font-medium">Min peak:</span>
            {[2, 3, 5, 8].map(n => (
              <button key={n} onClick={() => setMinGrowth(n)}
                className={`px-2.5 py-1 rounded-lg transition-colors ${minGrowth === n ? 'bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300 font-semibold' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}>{n}+</button>
            ))}
          </div>
          <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 ml-auto">
            <span className="font-medium">Sort:</span>
            {[{ id: 'peak', label: 'Peak funds' }, { id: 'velocity', label: 'Fastest' }, { id: 'recent', label: 'Recent' }].map(s => (
              <button key={s.id} onClick={() => setSortBy(s.id)}
                className={`px-2.5 py-1 rounded-lg transition-colors ${sortBy === s.id ? 'bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300 font-semibold' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}>{s.label}</button>
            ))}
          </div>
        </div>
      </div>
